...
def split_dataset(dataset, train_proportion = 0.7, test_proportion = 0.15, 
                    val_proportion = .15, shuffle=True, balance = False, buffer_size = 1000, seed = 42):
    ...
    validation = dataset.skip(test_size).take(val_size)
...
def get_spectogram_training(filename, downsampling_rate, frame_length_in_s, frame_step_in_s):
    ...
    #get spectrogram
    spectogram = get_spectrogram(audio_binary, downsampling_rate, frame_length, frame_step)

    return spectogram, label


def get_spectrogram(audio_binary, downsampling_rate, frame_length, frame_step):
